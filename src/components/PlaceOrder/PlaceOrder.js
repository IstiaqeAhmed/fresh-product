...
const PlaceOrder = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      "https://sleepy-stream-34221.herkuapp.com/orders?email=" +
        loggedUser.email,
      {
        method: "GET",
        headers: {
      ...
      .then((data) => {
        ...
        data.forEach((products) => {
          const total = data.reduce(
            (total, price) => total + parseFloat(price.products.price),
            0
          );
  ...
  const handleNext = () => {
    ...
    fetch("https://sleepy-stream-34221.herkuapp.com/addOrder", {
      method: "POST",
  ...
  return (
    <div className="container mt-2">
      <h3>
        Welcome back,{" "}
        {loggedInUser.name || loggedUser.name
          ? loggedInUser.name || loggedUser.name
          : loggedInUser.displayName || loggedUser.displayName}
        !
      </h3>
      ...
      <div className="d-flex justify-content-between mt-5">
        <h6 className="">You have ordered total : {orders.length} book/s </h6>{" "}
        <h6>Your total price is BDT {totalPrice}</h6>
      </div>
